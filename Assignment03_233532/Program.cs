    ...
    static void ReadXmlFile(string fileName)
    {
        ...
        using (XmlReader reader = XmlReader.Create(fileName))
        {
            while (reader.Read())
            {
                ...
                {
                    Console.Write($"{reader.Name}: "); // Print element name
                    ...
                    Console.WriteLine(); // Move to a new line after element name and attributes
                }

                // When text content of the element is encountered, print it
                if (reader.NodeType == XmlNodeType.Text)
                {
                    // Print the value on the same line as the element
                    Console.WriteLine(reader.Value); // Print the text content
                }
            }
        }
